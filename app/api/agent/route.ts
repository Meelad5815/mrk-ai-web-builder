...
function parseJson(text:string){const cleaned=text.replace(/^```json\s*/,'').replace(/```$/,'').trim();return JSON.parse(cleaned) as {summary?:string;changes?:Change[];warnings?:string[];plan?:string[]};}
...
export async function POST(request:Request){
 try{
  ...
  if(typeof repository!=='string'||!/^[^/]+\/[^/]+$/.test(repository)) return NextResponse.json({error:'Select a valid GitHub repository first.'},{status:400});
  ...
  const ref=await octokit.request('GET /repos/{owner}/{repo}/git/ref/{ref}',{owner,repo:name,ref:`heads/${meta.data.default_branch}`});
  const commitSha=(ref.data as any).object.sha as string;
  const commit=await octokit.request('GET /repos/{owner}/{repo}/git/commits/{commit_sha}',{owner,repo:name,commit_sha:commitSha});
  const tree=await octokit.request('GET /repos/{owner}/{repo}/git/trees/{tree_sha}',{owner,repo:name,tree_sha:(commit.data as any).tree.sha,recursive:'1'});
  const files=(tree.data.tree||[]).filter((x:any)=>x.type==='blob' && /\.(tsx?|jsx?|css|html|json|md|py|vue|php)$/i.test(x.path)).slice(0,18);
  const contexts:string[]=[];
  for(const f of files){try{const r=await octokit.request('GET /repos/{owner}/{repo}/contents/{path}',{owner,repo:name,path:f.path,ref:meta.data.default_branch});const d=r.data as any;if(typeof d.content==='string'){const content=Buffer.from(d.content.replace(/\n/g,''),'base64').toString('utf8');contexts.push(`FILE: ${f.path}\n${content.slice(0,12000)}`)}}catch{}}
  ...
  const safe=(parsed.changes||[]).filter(c=>c.path&&!c.path.startsWith('/')&&!c.path.split('/').includes('..')&&['create','modify','delete'].includes(c.action)).map(c=>({path:c.path,action:c.action,content:c.content}));
